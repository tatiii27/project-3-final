...
Promise.all([
  ...
  // JSON helpers (works with array or object-shaped files)
  ...
  // --- grid sizing for vertical gridlines like pic-1
  const axisY = height - 80;
  const gridHeight = height - 160; // space above x-axis to draw grid lines
  ...
  function updateChart(){
    ...
    // x scale / axis with vertical gridlines (pic-1 style)
    ...
    const axis = d3.axisBottom(xScale)
      ...
      .tickSize(-gridHeight)       // draw the vertical gridlines
      .tickSizeOuter(0);
    ...
    // bubbles (hover = ring highlight, not blue fill)
    ...
    node.enter().append("circle")
      ...
      .on("mouseover", function (event, d) {
        d3.select(this)
          ...
          .attr("r", size(d.price)*1.08)       // slight pop
          .attr("stroke","#3B82F6")            // ring color
          .attr("stroke-width", 4);            // thicker ring
