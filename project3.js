...
/* ==========================
   Annotation panel utils
   ========================== */
function ensurePanel(){ const p=d3.select("#annotations"); if(!p.empty()) return p; throw new Error("Missing <aside id='annotations'>"); }
...
Promise.all([
  ...
  // JSON helpers (works for array or object shapes)
  ...
  // layers: grid (behind) -> bubbles -> labels -> axis (front)
  ...
  function updateAnnotations({category, skin, maxPrice, filtered}){
    ...
    const head=d3.select("#anno-head"), tip=d3.select("#anno-tip"), budg=d3.select("#anno-budget"), comp=d3.select("#anno-compare");

    if (skin==="All" || category==="All" || !filtered?.length){
      panel.attr("hidden",true); head.text(""); tip.text(""); budg.text(""); comp.html(""); return;
    }
  ...
  function updateChart(){
    ...
    lines.exit().remove();
    gridG.lower();         // make sure grid is under everything

    /* === axis WITHOUT tick lines; with centered label + right arrow === */
    ...
    // price label (center) + arrow at far right
